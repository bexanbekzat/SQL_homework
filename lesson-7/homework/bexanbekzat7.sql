...
select MIN(Price) from Products5
...
select DepartmentName, AVG(Salary), COUNT(*) from Employees
group by DepartmentName
...
Select SaleDate, MONTH(SaleDate), SUM(SaleAmount), COUNT(distinct ProductID) from Sales
group by SaleDate, MONTH(SaleDate)
