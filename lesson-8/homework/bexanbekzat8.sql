...
insert into Customers10 (CustomerID, FirstName, LastName, Email, Phone, Address, City, State, PostalCode, Country) values
...
create table Products_Discounted10 (
    ProductID INT PRIMARY KEY,
    ProductName VARCHAR(100),
    Price DECIMAL(10, 2),
    Category VARCHAR(50),
...
insert into Products_Discounted10 values
...
insert into Sales10 (SaleID, ProductID, CustomerID, SaleDate, SaleAmount) values
...
1  select category, COUNT(*) as total_ProductName
from Products10
group by Category

2  select AVG(Price) from Products10
where Category = 'Electronics'

3  select * from Customers10
where City like 'L%'

4 select ProductName from Products10
where ProductName like '%er'

5 select * from Customers10
where Country like '%A'

6 select MAX(Price) from Products10
...
8 select Country, COUNT(*) from Customers10
group by Country

9 select MIN(Quantity), MAX(Quantity) from Orders

10 select  distinct o.CustomerID from Orders10 o left join Invoices i on o.CustomerID = i.CustomerID and i.InvoiceDate > = '2023-01-01' and i.InvoiceDate < '2023-02-01'
where i.InvoiceID is null


11 select ProductName from Products10 union all select ProductName from Products_Discounted10 


12 select ProductName from Products10 union select ProductName from Products_Discounted10


13 select  OrderDate AVG(TotalAmount) from Orders10
group by OrderDate
order by Orderdate
...
15 Select district_name, [2012], [2013] into Population_Each_Year from (select district_name,year,population from city_population) PIVOT(SUM(population) for year in ([2012], [2013]))



16 select SaleID, SUM(SaleAmount) from Sales10
group by SaleID


17 select ProductName from Products10
...
from(select City, Year, Population from City_Population ) as Source_Table
PIVOT(SUM(population) 
for City in ([Bektemir],[Chilonzor],[Yakkasaroy])) as Pivot_Table



19 select CustomerID, SUM(TotalAmount) from Invoices 
group by CustomerID 
order by Totalspent desc


20 select City, Year, population  
from Population_Each_Year
UNPIVOT 
    (Population for year in ([2012], [2013])) as UnpivotTable


21    select p.ProductName, COUNT(s.SaleID) as times_sold
   from Products p
   JOIN Sales s on p.ProductID = s.ProductID
   group by p.ProductName
   
  22 select Year, City, Population
   from Population_Each_City
   UNPIVOT
       (Population for City IN ([Bektemir], [Chilonzor], [Yakkasaroy])) as UnpivotTable
